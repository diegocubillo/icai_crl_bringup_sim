...
def generate_launch_description():
    ...
    bridge = Node(
        ...
        parameters=[{
            'config_file': os.path.join(pkg_project_bringup, 'config', 'kitt_md25_bridge_o1.yaml'),
        }],
        output='screen'
    )
    ...
    spawn_entity = Node(
        package='ros_gz_sim',
        executable='create',
        arguments=['-name', 'kitt_md25_o1',
                   '-x', '0.5',
                   '-y', '-0.65',
                   '-z', '0.84',
                   '-file', os.path.join(pkg_project_gazebo, 'models', 'md25_driver', 'kitt_md25_o1')],
        output='screen'
    )
